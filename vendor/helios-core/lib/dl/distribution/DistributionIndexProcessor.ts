...
export class DistributionIndexProcessor extends IndexProcessor {
    ...
    constructor(
        ...
        protected syncModpackWithServerEnabled = true,
        protected removeExcessiveModsEnabled = true
    ) {
        super(commonDir)
    }
    ...
    public async validate(onStageComplete: () => Promise<void>): Promise<{[category: string]: Asset[]}> {
        ...
        if(this.syncModpackWithServerEnabled && this.removeExcessiveModsEnabled) {
            await this.removeExtraMods(server.modules)
        }
    ...
    private async validateModules(modules: HeliosModule[], accumulator: Asset[]): Promise<void> {
        for(const module of modules) {
            if(!this.syncModpackWithServerEnabled && module.rawModule.type === Type.File) {
                continue
            }
